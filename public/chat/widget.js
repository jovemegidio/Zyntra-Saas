...
(function () {
  ...
  function init() {
    ...
    // Se é agente de suporte, mostrar painel diferente
    if (W.isSupportAgent) {
      initSupportPanel();
    } else {
      initLogin();
  ...
  function initSocket() {
    ...
    W.socket.on('support:queued', (data) => {
      showNotif(stripMd(data.message || 'Você está na fila de atendimento'));
    });
    ...
    W.socket.on('support:new-ticket', (ticket) => {
      ...
      if (!W.isOpen) {
        W.unreadCount++;
        updateBadge();
        showNotif('🎫 Novo chamado de ' + (ticket.userName || 'Usuário'));
      }
    });

    W.socket.on('support:accepted', (data) => {
      console.log('[Chat Suporte] Ticket aceito');
      // Mudar para tela de chat com o histórico
  ...
  function toggleWidget() {
    ...
    if (W.isOpen) {
      ...
      // Auto-login se tiver usuário logado e não for suporte
  ...
  function initLogin() {
    ...
    // Se tem usuário logado, preencher nome e esconder form
      ...
      <div class="afw-support-body">
        ...
        <div class="afw-support-queue" id="afw-support-queue">
          <div class="afw-support-empty">
            ...
            <span>Quando um usuário solicitar suporte, aparecerá aqui</span>
          </div>
        </div>
      </div>
      ...
      <div class="afw-support-ticket" data-ticket-id="${ticket.id}">
        <div class="afw-support-ticket-info">
          ...
          <div class="afw-support-ticket-details">
            <strong>${ticket.userName || 'Usuário'}</strong>
  ...
  function renderMessage(msg) {
    ...
    // No modo suporte, mensagens do usuário (não-bob, não-agente) são "received"
    ...
    if (isSystem) {
      ...
      div.innerHTML = '<div class="afw-msg-bubble">' +
        (!isSent ? '<div class="afw-msg-sender"><img src="/chat/BobAI.png" alt="BOB"> ' + (isBot ? 'BOB' : (msg.senderName || 'Usuário')) + '</div>' : '') +
  ...
  async function captureScreenshot() {
    try {
      ...
      if (e.name !== 'NotAllowedError') showNotif('⚠️ Não foi possível capturar a tela');
    }
  }
