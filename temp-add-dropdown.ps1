$ErrorActionPreference = 'Stop'
$root = "G:\Outros computadores\Meu laptop (2)\Zyntra"
Set-Location $root

$files = @(
    "modules/Faturamento/public/index.html",
    "modules/Faturamento/public/emitir.html",
    "modules/Faturamento/public/pix.html",
    "modules/Faturamento/public/danfe.html",
    "modules/Faturamento/public/consultar.html",
    "modules/Faturamento/public/dashboard.html",
    "modules/Faturamento/public/relatorios.html",
    "modules/Faturamento/public/nfse.html",
    "modules/Faturamento/public/eventos.html",
    "modules/Faturamento/public/logistica.html",
    "modules/Faturamento/public/regua.html",
    "modules/RH/public/pages/dashboard.html",
    "modules/RH/public/pages/funcionarios.html",
    "modules/RH/public/pages/dados-cadastrais.html",
    "modules/RH/public/pages/meus-holerites.html",
    "modules/RH/public/pages/calendario-rh.html",
    "modules/RH/public/pages/gestao-ponto.html",
    "modules/RH/public/pages/beneficios.html",
    "modules/RH/public/pages/gestao-solicitacoes.html",
    "modules/RH/public/pages/folha.html",
    "modules/RH/public/pages/requisicoes-compra.html",
    "modules/RH/public/pages/importar-ponto.html",
    "modules/RH/public/pages/manual-colaborador.html",
    "modules/RH/public/pages/importar-ponto_new.html",
    "modules/RH/public/pages/espelho-ponto.html",
    "modules/RH/public/pages/enviar-atestado.html",
    "modules/RH/public/pages/ponto.html",
    "modules/RH/public/pages/holerites.html",
    "modules/RH/public/pages/ferias.html",
    "modules/RH/public/pages/autoavaliacao.html",
    "modules/RH/public/pages/aplicar-avaliacoes.html",
    "modules/RH/public/pages/minhas-ferias.html",
    "modules/RH/public/pages/avaliacoes.html",
    "modules/RH/public/pages/avaliacoes-recebidas.html",
    "modules/RH/public/gestao-holerites.html",
    "modules/RH/public/areaadm.html",
    "modules/RH/public/funcionario.html",
    "modules/RH/public/treinamentos.html",
    "modules/RH/public/solicitacoes.html",
    "modules/RH/public/dados-pessoais.html",
    "modules/Financeiro/index.html",
    "modules/Financeiro/relatorios.html",
    "modules/Financeiro/bancos.html",
    "modules/Financeiro/contas-receber.html",
    "modules/Financeiro/contas-pagar.html",
    "modules/Financeiro/orcamentos.html",
    "modules/Financeiro/plano-contas.html",
    "modules/Financeiro/fluxo-caixa.html",
    "modules/Financeiro/conciliacao.html",
    "modules/Financeiro/impostos.html",
    "modules/Financeiro/centros-custo.html",
    "modules/Financeiro/dashboard-contas-pagar.html",
    "modules/Financeiro/dashboard-contas-receber.html",
    "modules/Financeiro/boletos.html",
    "modules/Financeiro/nfse.html",
    "modules/Financeiro/recorrencias.html",
    "modules/Financeiro/public/contas_pagar.html",
    "modules/Financeiro/public/contas_receber.html",
    "modules/Financeiro/public/index.html",
    "modules/Financeiro/public/contas_bancarias.html",
    "modules/Financeiro/public/relatorios.html",
    "modules/Financeiro/public/fluxo_caixa.html",
    "modules/NFe/logistica.html",
    "modules/NFe/index.html",
    "modules/NFe/danfe.html",
    "modules/NFe/emitir.html",
    "modules/NFe/nfse.html",
    "modules/NFe/consultar.html",
    "modules/NFe/relatorios.html",
    "modules/NFe/inutilizacao.html",
    "modules/NFe/eventos.html",
    "modules/PCP/ordens-producao.html",
    "modules/PCP/pages/estoque.html",
    "modules/PCP/index_new.html",
    "modules/PCP/pages/faturamento.html",
    "modules/PCP/pages/ordem-compra.html",
    "modules/PCP/pages/relatorios.html",
    "modules/PCP/pages/materiais.html",
    "modules/PCP/sistema_corrigido_final.html",
    "modules/Compras/requisicoes.html",
    "modules/Compras/recebimento.html",
    "modules/Compras/relatorios.html",
    "modules/Compras/gestao-estoque.html",
    "modules/Compras/index.html",
    "modules/Compras/fornecedores.html",
    "modules/Compras/cotacoes.html",
    "modules/Compras/pedidos.html",
    "modules/Admin/public/pages/permissoes.html",
    "modules/Consultoria/acesso.html"
)

$scriptTag = '    <script src="/js/user-dropdown.js?v=20260319" defer></script>'
$ok = 0; $skip = 0; $fail = 0

foreach ($f in $files) {
    $fullPath = Join-Path $root $f
    if (-not (Test-Path $fullPath)) {
        Write-Host "NOTFOUND: $f"
        $fail++
        continue
    }
    $content = [System.IO.File]::ReadAllText($fullPath, [System.Text.Encoding]::UTF8)
    if ($content -match 'user-dropdown\.js') {
        Write-Host "ALREADY: $f"
        $skip++
        continue
    }
    if ($content -match '</body>') {
        $newContent = $content -replace '</body>', "$scriptTag`r`n</body>"
        [System.IO.File]::WriteAllText($fullPath, $newContent, [System.Text.Encoding]::UTF8)
        Write-Host "OK: $f"
        $ok++
    } else {
        Write-Host "NOBODY: $f"
        $fail++
    }
}

Write-Host "`nResult: $ok updated, $skip already had it, $fail failed/skipped"
