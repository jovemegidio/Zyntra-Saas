/**
 * ALUFORCE CHAT WIDGET — JavaScript
 * Widget flutuante com BOB IA, áudio e suporte
 */
...
(function () {
  ...
  function initSocket() {
    ...
    W.socket.on('support:queued', (data) => {
      showNotif(stripMd(data.message));
    });
    ...
    W.socket.on('support:connected', (data) => {
      ...
      if (av) { av.src = ''; av.style.background = 'linear-gradient(135deg,#4caf50,#66bb6a)'; av.style.padding = '6px'; av.alt = '🎧'; }
    });

    W.socket.on('support:auto-transfer', (data) => {
      W.socket.emit('support:request', {
        conversationId: data.conversationId || W.conversationId,
        reason: 'Transferência solicitada pelo usuário via BOB'
      });
    });
  }
  ...
  // ==================== LOGIN ====================
  function initLogin() {
    $('afw-login-form').addEventListener('submit', (e) => {
      e.preventDefault();
  ...
  function doLogin(name) {
    W.socket.emit('user:register', { name });
    W.isLoggedIn = true;
    ...
    setTimeout(() => {
      ...
      // Se tinha um tópico rápido pendente
      if (W.pendingTopic) {
        setTimeout(() => {
  ...
  function renderMessage(msg) {
    ...
    if (isSystem) {
      ...
      div.innerHTML = `<div class="afw-msg-bubble">
        ${!isSent ? `<div class="afw-msg-sender"><img src="BobAI.png" alt="BOB"> ${isBot ? 'BOB' : esc(msg.senderName)}</div>` : ''}
      ...
      const fname = esc(msg.metadata?.originalName || 'Arquivo');
      const isPdf = /\.pdf$/i.test(msg.metadata?.originalName || '');
      const previewId = 'pdfPrev_' + msg.id;
      div.innerHTML = `<div class="afw-msg-bubble">
        ${!isSent ? `<div class="afw-msg-sender"><img src="BobAI.png" alt="BOB"> ${isBot ? 'BOB' : esc(msg.senderName)}</div>` : ''}
        <div class="afw-file-msg">
          <div class="afw-file-icon"><i class="fas fa-${icon}"></i></div>
          <div class="afw-file-info">
            <div class="afw-file-name">${fname}</div>
            <div class="afw-file-size">${fmtSize(msg.metadata?.size || 0)}</div>
          </div>
        ...
        ${isPdf ? `<div class="afw-pdf-inline-preview" id="${previewId}"><iframe src="${msg.content}#toolbar=0" title="PDF"></iframe></div>` : ''}
        <div class="afw-msg-meta"><span class="afw-msg-time">${time}</span>${isSent ? '<span class="afw-msg-status"><i class="fas fa-check"></i></span>' : ''}</div>
      </div>`;
    } else {
      div.innerHTML = `<div class="afw-msg-bubble">
  ...
  async function startRec() {
    try {
      ...
      W.mediaRecorder = new MediaRecorder(stream, { mimeType: 'audio/webm;codecs=opus' });
      W.audioChunks = [];
      ...
      W.recTimer = setInterval(updateRecTime, 1000);
      updateRecTime();
    } catch (e) {
      showNotif('⚠️ Permissão de microfone negada');
    }
  }

  function cancelRec() {
    if (W.mediaRecorder && W.mediaRecorder.state !== 'inactive') {
  ...
  function stopAndSendRec() {
    if (!W.mediaRecorder || W.mediaRecorder.state === 'inactive') return;
    W.mediaRecorder.onstop = async () => {
      ...
      try {
        ...
        if (data.url) {
          W.socket.emit('audio:send', { conversationId: W.conversationId, audioUrl: data.url, duration: dur, mimetype: 'audio/webm' });
      ...
      } catch (e) { showNotif('❌ Erro ao enviar áudio'); }
      W.mediaRecorder.stream.getTracks().forEach(t => t.stop());
    };
    W.mediaRecorder.stop();
    W.isRecording = false;
  ...
  async function uploadAndSendFile(file) {
    // Mostrar preview temporário para imagens
    ...
    try {
      ...
      if (data.url) {
        ...
        // Se já renderizou preview, atualizar a src. Senão, renderizar
    ...
    } catch (e) { showNotif('❌ Erro ao enviar arquivo'); }
  }
  ...
  function initAttachMenu() {
    ...
    // Opção: Arquivo
    $('afw-attach-file').addEventListener('click', () => {
      menu.classList.add('hidden');
      $('afw-file-input').click();
    });

    // Opção: Foto / Imagem
    $('afw-attach-photo').addEventListener('click', () => {
      menu.classList.add('hidden');
      $('afw-photo-input').click();
    });

    // Opção: Screenshot
    $('afw-attach-screenshot').addEventListener('click', () => {
      menu.classList.add('hidden');
      captureScreenshot();
    });
  ...
  function initScreenshot() {
    // Nada a inicializar, a lógica fica no captureScreenshot
  }

  async function captureScreenshot() {
    try {
      // Minimizar widget para capturar a tela limpa
      const widgetWindow = $('afw-window');
      const fab = $('afw-fab');
      widgetWindow.style.visibility = 'hidden';
      fab.style.visibility = 'hidden';

      // Usar getDisplayMedia para capturar tela
      const stream = await navigator.mediaDevices.getDisplayMedia({
        video: { mediaSource: 'screen', cursor: 'always' },
        audio: false
      });

      // Capturar frame do stream
      const track = stream.getVideoTracks()[0];
      const imageCapture = new ImageCapture(track);

      // Pequeno delay para garantir que o frame está pronto
      ...
      // Restaurar widget em caso de erro ou cancelamento
      $('afw-window').style.visibility = '';
      $('afw-fab').style.visibility = '';

      if (e.name !== 'NotAllowedError') {
        showNotif('⚠️ Não foi possível capturar a tela');
      }
    }
  }
  ...
  function showScreenshotPreview(blob) {
    ...
    overlay.innerHTML = `
      <div class="afw-screenshot-header">
        <span><i class="fas fa-desktop"></i> Captura de Tela</span>
        <div class="afw-screenshot-actions">
          <button class="afw-screenshot-btn cancel" id="afw-ss-cancel"><i class="fas fa-times"></i> Cancelar</button>
          <button class="afw-screenshot-btn send" id="afw-ss-send"><i class="fas fa-paper-plane"></i> Enviar</button>
        </div>
      </div>
      <div class="afw-screenshot-img-wrap">
        <img src="${url}" alt="Screenshot">
      </div>
    `;

    // Inserir na área de mensagens
    const chatEl = $('afw-chat');
    chatEl.appendChild(overlay);

    // Cancelar
    overlay.querySelector('#afw-ss-cancel').addEventListener('click', () => {
  ...
  function initSupport() {
    $('afw-btn-support').addEventListener('click', requestSupport);
    $('afw-btn-yes-support').addEventListener('click', () => { $('afw-support-bar').classList.add('hidden'); requestSupport(); });
    $('afw-btn-no-support').addEventListener('click', () => { $('afw-support-bar').classList.add('hidden'); sendText('Não, obrigado. Vou tentar reformular minha pergunta.'); });
  }

  function requestSupport() {
    W.socket.emit('support:request', { conversationId: W.conversationId, reason: 'Usuário solicitou atendimento humano' });
  }
  ...
  function applyModalZoom() {
    ...
    if (!pdf.classList.contains('hidden')) {
      pdf.style.transform = `scale(${modalZoom})`;
    }
  }
  ...
  function initTheme() {
    ...
    // Botão na brand bar (tela de boas-vindas)
  ...
  // FIX-XSS: Escape HTML before applying markdown to prevent stored/reflected XSS
  function esc(s) {
  ...
  function genWave() {
    let b = '';
    for (let i = 0; i < 28; i++) b += `<div class="bar" style="height:${Math.random() * 16 + 3}px"></div>`;
    return b;
  }

  function fileIcon(name) {
    const ext = name.split('.').pop().toLowerCase();
    const map = { pdf: 'file-pdf', doc: 'file-word', docx: 'file-word', xls: 'file-excel', xlsx: 'file-excel', zip: 'file-archive', rar: 'file-archive' };
    return map[ext] || 'file';
  }
