...
// 🛡️ v8.0: Structured 403 violation logger
function _log403(userId, userEmail, method, path, code, detail) {
    const ts = new Date().toISOString();
    console.warn(`[RBAC-VIOLATION] ${ts} | userId=${userId} | email=${userEmail || '?'} | ${method} ${path} | code=${code} | ${detail}`);
}
    ...
    jwt.verify(token, JWT_SECRET, { algorithms: ['HS256'] }, async (err, user) => {
        ...
        if (user.jti && _cacheService) {
            try {
                ...
                // Cache indisponível para verificar blacklist — permitir acesso com warning
                // JWT já foi verificado (assinatura + expiração), blacklist é camada extra
                // Bloquear tudo quando Redis cai brevemente causa instabilidade no sistema
                console.warn('[AUTH] Cache indisponível para verificar blacklist — permitindo acesso (JWT válido):', cacheErr.message);
            }
        }
...
function requireAdmin(req, res, next) {
    ...
    _log403(req.user.id, req.user.email, req.method, req.originalUrl, 'ADMIN_REQUIRED', 'admin check failed');
    ...
    if (pool) {
        permissionService.hasModuleAccess(pool, req.user.id, 'rh', req.user)
            .then(hasAccess => {
                ...
                _log403(req.user.id, req.user.email, req.method, req.originalUrl, 'ADMIN_RH_REQUIRED', 'admin/rh check failed');
                return res.status(403).json({
            ...
            .catch(() => {
                _log403(req.user.id, req.user.email, req.method, req.originalUrl, 'ADMIN_RH_REQUIRED', 'admin/rh DB check error');
                res.status(403).json({
                    message: 'Acesso negado. Requer privilégios de administrador ou RH.',
                    code: 'ADMIN_RH_REQUIRED'
                });
        ...
        _log403(req.user.id, req.user.email, req.method, req.originalUrl, 'ADMIN_RH_REQUIRED', 'no pool available');
        return res.status(403).json({
            message: 'Acesso negado. Requer privilégios de administrador ou RH.',
            code: 'ADMIN_RH_REQUIRED'
        });
...
function requireModule(module) {
    return async (req, res, next) => {
        ...
        _log403(req.user.id, req.user.email, req.method, req.originalUrl, 'MODULE_DENIED', `module=${module}`);
        return res.status(403).json({
...
function requireAction(module, actions) {
    return async (req, res, next) => {
        ...
        _log403(req.user.id, req.user.email, req.method, req.originalUrl, 'ACTION_DENIED', `module=${module} actions=${actionsArray.join(',')}`);
        return res.status(403).json({
...
function checkModuleAccess(moduloCodigo, tipoPermissao = 'visualizar') {
    return async (req, res, next) => {
        ...
        const pool = req.app?.locals?.pool;
        if (!pool) {
            _log403(req.user.id, req.user.email, req.method, req.originalUrl, 'MODULE_ACCESS_DENIED', `module=${moduloCodigo} no_pool`);
            return res.status(403).json({
        ...
        _log403(req.user.id, req.user.email, req.method, req.originalUrl, 'MODULE_ACCESS_DENIED', `module=${moduloCodigo} perm=${tipoPermissao}`);
        return res.status(403).json({
            message: `Acesso negado ao módulo ${moduloCodigo}`,
            code: 'MODULE_ACCESS_DENIED'
        });
...
function writeGuard(req, res, next) {
    ...
    // AUDIT-FIX R2: Se flags não foram setadas, NEGAR por padrão (deny-by-default)
    // Middleware anterior (requireModule) DEVE setar as flags; se não setou, é bypass
    if (req.canEdit === undefined && req.canCreate === undefined && req.canDelete === undefined) {
        console.warn(`[WRITE_GUARD] Flags de permissão não definidas para ${req.method} ${req.originalUrl} — user ${req.user?.id}. NEGANDO por segurança.`);
        _log403(req.user?.id, req.user?.email, req.method, req.originalUrl, 'WRITE_GUARD_NO_FLAGS', 'permission flags not set');
        return res.status(403).json({ message: 'Permissões não carregadas. Acesso negado.', code: 'WRITE_GUARD_NO_FLAGS' });
    }

    if (req.method === 'DELETE' && req.canDelete === false) {
        _log403(req.user?.id, req.user?.email, req.method, req.originalUrl, 'WRITE_GUARD_DELETE', 'delete blocked');
    ...
    if (req.method === 'POST' && req.canCreate === false) {
        _log403(req.user?.id, req.user?.email, req.method, req.originalUrl, 'WRITE_GUARD_CREATE', 'create blocked');
    ...
    if (['PUT', 'PATCH'].includes(req.method) && req.canEdit === false) {
        _log403(req.user?.id, req.user?.email, req.method, req.originalUrl, 'WRITE_GUARD_EDIT', 'edit blocked');
