...
// AUDIT-FIX R3: Adicionado authenticateToken antes de authorizeAdmin
app.post('/api/admin/cache/invalidate', authenticateToken, authorizeAdmin, express.json(), async (req, res) => {
    try {
...
// AUDIT-FIX R3: Adicionado authenticateToken antes de authorizeAdmin
app.post('/api/admin/compute-aggregates', authenticateToken, authorizeAdmin, express.json(), async (req, res) => {
    try {
...
apiVendasRouter.patch('/pedidos/:id', async (req, res, next) => {
    try {
        ...
        if (updates.status !== undefined) {
            const validStatuses = ['orcamento', 'orçamento', 'analise', 'analise-credito', 'aprovado', 'pedido-aprovado', 'faturar', 'faturado', 'entregue', 'cancelado', 'recibo', 'credito-reprovado'];
...
apiVendasRouter.post('/pedidos/:id/aprovacao-credito', async (req, res, next) => {
    try {
        ...
        // Only admin + authorized approvers can perform credit analysis
        const userName = (user.nome || user.email || '').toLowerCase().split(' ')[0];
        const isAdmin = user.role === 'admin' || user.is_admin;
        const approvers = ['thiago', 'guilherme', 'clemerson'];
        if (!isAdmin && !approvers.includes(userName)) {
            return res.status(403).json({ success: false, message: 'Apenas administradores e aprovadores autorizados podem realizar análise de crédito.' });
        }
...
apiVendasRouter.get('/clientes/:id/details', async (req, res, next) => {
    ...
    apiVendasRouter.get('/clientes/:id/credito', async (req, res, next) => {
        try {
            ...
            if (clienteRows.length > 0) {
                ...
                const [empRows] = await pool.query(
                    'SELECT id, nome, razao_social, limite_credito FROM empresas WHERE id = ? LIMIT 1',
                    [clienteId]
                );
                if (empRows.length > 0) cliente = empRows[0];
            }
            if (!cliente) return res.status(404).json({ message: 'Cliente não encontrado.' });

            const limiteCredito = parseFloat(cliente.limite_credito || 0);
            const lookupId = cliente.empresa_id || cliente.id;

            const [creditUsed] = await pool.query(
                `SELECT COALESCE(SUM(valor), 0) as total_pendente
                 FROM pedidos
                 WHERE (cliente_id = ? OR empresa_id = ?)
...
// AUDIT-FIX R3: Adicionado authenticateToken — antes vazava TODAS as notificações sem auth
