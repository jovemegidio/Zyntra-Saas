...
module.exports = function createRHRequisicoesCompraRoutes(deps) {
    ...
    // PUT /api/rh/requisicoes-compra/:id (alias: /api/rh/pedidos/:id) — editar
    router.put(['/requisicoes-compra/:id', '/pedidos/:id'], authenticateToken, async (req, res) => {
        const { id } = req.params;
        const { solicitante, departamento, data_necessaria, prioridade, observacoes, itens } = req.body || {};
        ...
        try {
            ...
            const [existing] = await conn.query('SELECT id, status FROM rh_requisicoes_compra WHERE id = ?', [id]);
            if (!existing.length) {
                await conn.rollback();
            ...
            await conn.query(`
                UPDATE rh_requisicoes_compra
                SET solicitante = COALESCE(?, solicitante),
                    departamento = COALESCE(?, departamento),
                    data_necessaria = COALESCE(?, data_necessaria),
                    prioridade = COALESCE(?, prioridade),
                    observacoes = COALESCE(?, observacoes),
                    updated_at = NOW()
                WHERE id = ?
            `, [solicitante || null, departamento || null, data_necessaria || null, prioridade || null, observacoes || null, id]);

            let valorTotal = existing[0].valor_total || 0;
            const [pedidoCompraRows] = await conn.query(
                "SELECT id FROM pedidos_compra WHERE origem = 'RH' AND ordem_compra_pcp_id = ? LIMIT 1",
                [id]
            );
            const pedidoCompraId = pedidoCompraRows[0]?.id || null;

            if (Array.isArray(itens) && itens.length) {
                await conn.query('DELETE FROM rh_requisicoes_compra_itens WHERE requisicao_id = ?', [id]);
                if (pedidoCompraId) {
                    await conn.query('DELETE FROM itens_pedido WHERE pedido_id = ?', [pedidoCompraId]);
                }
                ...
                for (const it of itens) {
                    ...
                    await conn.query(`
                        INSERT INTO rh_requisicoes_compra_itens
                            (requisicao_id, descricao, quantidade, unidade, preco_unitario, preco_total)
                        VALUES (?, ?, ?, ?, ?, ?)
                    `, [id, it.descricao, qtd, it.unidade || 'UN', preco, sub]);
                    ...
                    if (pedidoCompraId) {
                        await conn.query(`
                            INSERT INTO itens_pedido
                                (pedido_id, codigo_produto, descricao, quantidade, unidade, preco_unitario, preco_total, observacoes)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                        `, [pedidoCompraId, `RH-${id}`, it.descricao, qtd, it.unidade || 'UN', preco, sub, solicitante || req.user?.nome || 'RH']);
                    }
                }

                await conn.query('UPDATE rh_requisicoes_compra SET valor_total = ? WHERE id = ?', [valorTotal, id]);
            }

            if (pedidoCompraId) {
                await conn.query(`
                    UPDATE pedidos_compra
                    SET data_entrega_prevista = COALESCE(?, data_entrega_prevista),
                        valor_total = ?,
                        valor_final = ?,
                        observacoes = ?,
                        updated_at = NOW()
                    WHERE id = ?
                `, [data_necessaria || null, valorTotal, valorTotal, `[RH - ${departamento || 'N/A'}] ${observacoes || 'Sem observações'}`, pedidoCompraId]);
            }

            await conn.commit();
            res.json({ success: true, message: 'Requisição atualizada com sucesso' });
        } catch (err) {
            await conn.rollback();
            console.error('[RH-REQUISICOES] Erro ao editar requisição:', err);
    ...
    // DELETE /api/rh/requisicoes-compra/:id (alias: /api/rh/pedidos/:id) — cancelamento/exclusão lógica
    router.delete(['/requisicoes-compra/:id', '/pedidos/:id'], authenticateToken, async (req, res) => {
        try {
            const { id } = req.params;
            const [rows] = await pool.query('SELECT id, status FROM rh_requisicoes_compra WHERE id = ?', [id]);
            if (!rows.length) {
                return res.status(404).json({ error: 'Requisição não encontrada' });
            }

            if (['aprovada', 'concluida'].includes(String(rows[0].status || '').toLowerCase())) {
                return res.status(409).json({ error: 'Esta requisição já está vinculada ao fluxo de compras e não pode ser excluída.' });
            }

            await pool.query("UPDATE rh_requisicoes_compra SET status = 'cancelado', updated_at = NOW() WHERE id = ?", [id]);
            await pool.query("UPDATE pedidos_compra SET status = 'cancelado' WHERE origem = 'RH' AND ordem_compra_pcp_id = ?", [id]);

            res.json({ success: true, message: 'Requisição cancelada com sucesso' });
        } catch (err) {
            console.error('[RH-REQUISICOES] Erro ao excluir/cancelar requisição:', err);
