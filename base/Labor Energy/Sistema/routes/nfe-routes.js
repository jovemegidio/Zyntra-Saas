...
module.exports = function createNfeRoutes(deps) {
    ...
    router.post('/emitir', authenticateToken, async (req, res, next) => {
        ...
        try {
            ...
            const { cliente_id, servico_id, descricao_servico, valor, vencimento, pedido_id, itens } = req.body;

            // AUDIT-FIX: Validar valor é numérico e positivo
            const valorNum = parseFloat(valor);
            if (!Number.isFinite(valorNum) || valorNum <= 0) {
                await connection.rollback();
                connection.release();
                return res.status(400).json({ error: 'Valor deve ser numérico e positivo' });
            }
    
            // Validar cliente
            const [cliente] = await connection.query('SELECT id, municipio FROM clientes WHERE id = ?', [cliente_id]);
            if (cliente.length === 0) {
                await connection.rollback();
                connection.release();
            ...
            // AUDIT-FIX: Recalcular impostos server-side (não confiar no client)
            const municipio = cliente[0].municipio || '';
            const impostos = {
                ISS: municipio.toUpperCase().includes('SP') ? valorNum * 0.05 : valorNum * 0.03,
                PIS: valorNum * 0.0065,
                COFINS: valorNum * 0.03,
                CSLL: valorNum * 0.01,
                IRRF: valorNum * 0.015
            };
    
            // AUDIT-FIX R2: Gerar número sequencial NFe via serviço compartilhado
            let numeroNfe = null;
            try {
                const nfNumero = await faturamentoShared.gerarProximoNumeroNFe(connection);
                numeroNfe = nfNumero.numero;
            } catch (seqErr) {
                console.warn('[NFE-EMITIR] Falha ao gerar numero_nfe sequencial:', seqErr.message);
            }

            // Emitir NF-e
            const [nfeResult] = await connection.query(
                'INSERT INTO nfe (cliente_id, servico_id, descricao_servico, valor, impostos, status, data_emissao, numero) VALUES (?, ?, ?, ?, ?, "autorizada", NOW(), ?)',
                [cliente_id, servico_id, descricao_servico, valorNum, JSON.stringify(impostos), numeroNfe]
            );
    ...
    router.post('/cancelar/:nfe_id', authenticateToken, authorizeAdmin, async (req, res, next) => {
        ...
        try {
            ...
            // AUDIT-FIX R2: Prazo fiscal — não permitir cancelamento após 24h da autorização/emissão
            const dataRef = nfe[0].data_autorizacao || nfe[0].data_emissao;
            if (dataRef) {
                const horasDecorridas = (Date.now() - new Date(dataRef).getTime()) / (1000 * 60 * 60);
                if (horasDecorridas > 24) {
                    await connection.rollback();
                    return res.status(400).json({
                        error: `NF-e não pode ser cancelada após 24 horas da autorização (${Math.floor(horasDecorridas)}h decorridas). Use Carta de Correção.`,
                        code: 'NFE_CANCEL_PRAZO_EXPIRADO'
                    });
                }
            }
