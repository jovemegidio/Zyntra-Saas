...
const authLimiter = rateLimit({
    windowMs: 15 * 60 * 1000, // 15 minutos
    max: isDev ? 200 : 100, // Limite de requests por IP (liberal pois lockout por conta é feito no handler)
    ...(authStore ? { store: authStore } : {}),
    // Se o store falhar, permitir o request (o lockout por conta no handler protege contra brute-force)
    passOnStoreError: true,
    message: {
        error: 'Muitas requisições deste endereço IP. Tente novamente em alguns minutos.',
        retryAfter: '15 minutos',
        code: 'AUTH_RATE_LIMIT'
    },
    skipSuccessfulRequests: true, // Conta apenas tentativas falhas — logins bem-sucedidos não consomem quota
    standardHeaders: true,
    legacyHeaders: false,
    validate: { xForwardedForHeader: false }, // Desabilita validação - usamos trust proxy
    keyGenerator: (req) => {
