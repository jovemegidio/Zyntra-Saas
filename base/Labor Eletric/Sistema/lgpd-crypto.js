 ...
 * AUDIT-FIX S10.1: Re-enabled AES-256-GCM encryption for LGPD compliance.
 * Legacy data without 'ENC:' prefix is treated as plaintext by decryptPII.
 * @param {string} plaintext - The value to store
 * @returns {string} Encrypted string with prefix 'ENC:iv:authTag:ciphertext'
 */
function encryptPII(plaintext) {
    ...
    try {
        ...
        console.error('[LGPD] Encryption failed, storing plaintext:', err.message);
        return plaintext; // Fallback to plaintext to avoid data loss
    }
}
