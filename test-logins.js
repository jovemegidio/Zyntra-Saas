        ...
        const req = http.request({
            ...
            res.on('end', () => {
                try {
                    ...
                    console.log(`${login.email}: ${res.statusCode} - success=${j.success} user=${j.user?.nome || 'N/A'} role=${j.user?.perfil || j.user?.role || 'N/A'}`);
                } catch (e) {
                    console.log(`${login.email}: ${res.statusCode} - ${body.substring(0, 100)}`);
                }
                resolve();
            });
        });
