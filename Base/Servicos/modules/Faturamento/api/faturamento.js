...
module.exports = (pool, authenticateToken) => {
    ...
    router.post('/nfe/webhook/status', async (req, res) => {
        try {
            ...
            if (!identificador) {
                console.warn('[NFe-WEBHOOK] ⚠️ Evento recebido sem identificador. Payload descartado.');
            ...
            const nfe = nfes[0];
            const statusMap = { 'autorizada': 'autorizada', 'aprovada': 'autorizada', 'cancelada': 'cancelada', 'rejeitada': 'rejeitada', 'denegada': 'rejeitada', 'processando': 'processando' };
            ...
            if (statusInterno) {
                ...
                console.log(`[NFe-WEBHOOK] ✅ NFe #${nfe.numero_nfe || nfe.id} atualizada: ${nfe.status} → ${statusInterno} (${result.affectedRows} linha(s))`);
            }
