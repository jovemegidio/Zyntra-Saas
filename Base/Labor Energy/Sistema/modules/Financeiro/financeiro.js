...
async function carregarPermissoes() {
    try {
        ...
        console.warn('⚠️ Falha ao carregar permissões — acesso restrito (deny-by-default)');
        // AUDIT-FIX: Fallback deny-by-default (antes era full-access)
...
function editarConta(id, tipo) {
    ...
    setVal('conta-vencimento', formatarDataParaInput(conta.vencimento));
...
function fecharModal(modalId) {
    // Se modalId for passado, usar ele
    // Se não, fechar o modal ativo (modalConta)
    let modal;
    if (modalId) {
...
function formatarData(data) {
    if (!data) return '-';
    try {
        // Se a data já é um objeto Date ou ISO string completa
