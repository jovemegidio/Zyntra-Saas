...
app.post('/api/financeiro/contas-pagar/:id/baixar', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // AUDIT-FIX R2: Guard de idempotência com FOR UPDATE (previne pagamento duplo)
        const [currentConta] = await connection.execute(
            'SELECT status FROM contas_pagar WHERE id = ? FOR UPDATE', [id]
        );
        if (!currentConta.length) {
            await connection.rollback();
            return res.status(404).json({ error: 'Conta não encontrada' });
        }
        if (['pago', 'liquidada'].includes((currentConta[0].status || '').toLowerCase())) {
            await connection.rollback();
            return res.status(409).json({ error: 'Esta conta já foi paga. Não é possível pagar novamente.' });
        }
...
app.delete('/api/financeiro/contas-pagar/:id', authenticateToken, requireFinancePermission('excluir'), async (req, res) => {
    try {
        ...
        // FIX-100: Soft-delete instead of hard DELETE
        await pool.execute('UPDATE contas_pagar SET status = \'cancelado\', deleted_at = NOW() WHERE id = ?', [id]);
...
app.post('/api/financeiro/contas-receber/importar-excel', authenticateToken, upload.single('arquivo'), async (req, res) => {
    try {
        ...
        // AUDIT-FIX R2-CRIT-01: Envolver DELETE+INSERTs em transação + remover OR perigoso
        const importConnection = await pool.getConnection();
        await importConnection.beginTransaction();
        
        try {
        // Delete ONLY previous excel imports (safe — no OR clause that could delete manual records)
        const [delResult] = await importConnection.execute(
            `DELETE FROM contas_receber WHERE origem_integracao = 'importacao_excel'`);
        ...
        for (let i = headerRowIdx + 1; i < rawData.length; i++) {
            ...
            try {
                ...
                const valorLiquido = Math.round((valor - pis - cofins - csll - ir - iss - inss) * 100) / 100;
...
app.delete('/api/financeiro/contas-receber/:id', authenticateToken, requireFinancePermission('excluir'), async (req, res) => {
    try {
        ...
        // FIX-100: Soft-delete instead of hard DELETE
        await pool.execute('UPDATE contas_receber SET status = \'cancelado\', deleted_at = NOW() WHERE id = ?', [id]);
        res.json({ success: true, message: 'Conta excluída com sucesso' });
    } catch (error) {
...
app.post('/api/financeiro/contas-receber/:id/baixar', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // AUDIT-FIX R3: Adicionado FOR UPDATE para prevenir race condition (baixa duplicada)
    ...
    try {
        ...
        app.delete('/api/financeiro/anexos/:id', authenticateToken, requireFinancePermission('excluir'), async (req, res) => {
            try {
                ...
                // FIX-100: Soft-delete instead of hard DELETE
                await pool.execute('UPDATE financeiro_anexos SET deleted_at = NOW() WHERE id = ?', [id]);
