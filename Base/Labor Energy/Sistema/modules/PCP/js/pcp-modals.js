...
// ============================================
// GUARD DE ACESSO: operadores (role='operador') so podem acessar
// Ordens de Producao e Apontamentos -- redirecionar das paginas restritas
// ============================================
(function verificarAcessoPCPOperador() {
    var path = window.location.pathname;
    if (path.indexOf('/PCP/pages/') === -1 && path.indexOf('/modules/PCP/pages/') === -1) return;
    fetch('/api/me', { credentials: 'include' })
        .then(function(resp) { return resp.ok ? resp.json() : null; })
        .then(function(user) {
            if (!user) return;
            var role = ((user.role || user.cargo || '') + '').toLowerCase();
            var roles = (user.roles || []).map(function(r) { return r.toLowerCase(); });
            var fullAccess = ['admin', 'administrador', 'supervisor', 'gerente', 'diretoria', 'ti', 'pcp', 'analista'];
            var isFullAccess = fullAccess.some(function(r) { return role.indexOf(r) !== -1; }) ||
                               roles.some(function(r) { return fullAccess.some(function(a) { return r.indexOf(a) !== -1; }); });
            if (!isFullAccess) { window.location.replace('../apontamentos.html'); }
        })
        .catch(function() {});
})();
