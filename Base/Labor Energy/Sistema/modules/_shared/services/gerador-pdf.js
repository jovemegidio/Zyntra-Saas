...
class GeradorPDF {
    constructor(config = {}) {
        this.empresa = config.empresa || {
            nome: process.env.EMPRESA_NOME || 'ZYNTRA ERP',
            cnpj: process.env.EMPRESA_CNPJ || '00.000.000/0001-00',
            endereco: process.env.EMPRESA_ENDERECO ? `${process.env.EMPRESA_ENDERECO} - ${process.env.EMPRESA_BAIRRO || ''}` : 'Endereço não configurado',
            cidade: process.env.EMPRESA_CIDADE ? `${process.env.EMPRESA_CIDADE} - ${process.env.EMPRESA_UF || ''}` : '',
