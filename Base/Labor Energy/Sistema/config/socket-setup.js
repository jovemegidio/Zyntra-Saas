...
const DEV_SOCKET_BYPASS = process.env.SOCKET_ALLOW_DEV_BYPASS === 'true';
...
function setupSocketIO(httpServer, { Server, allowedOrigins, JWT_SECRET, pool }) {
    const io = new Server(httpServer, {
        cors: {
            origin: function(origin, callback) {
                ...
                if (DEV_SOCKET_BYPASS) {
                    console.warn(`⚠️ Socket.IO CORS liberado temporariamente via SOCKET_ALLOW_DEV_BYPASS para: ${origin}`);
                    return callback(null, true);
                }
    ...
    io.use((socket, next) => {
        ...
        if (!token) {
            if (DEV_SOCKET_BYPASS) return next();
