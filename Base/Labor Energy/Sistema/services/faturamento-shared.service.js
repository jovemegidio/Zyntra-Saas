...
class FaturamentoSharedService {
    ...
    async determinarCFOP(tipoOperacao, ufEmpresa, ufCliente, cfopManual = null) {
        if (cfopManual) {
            // AUDIT-FIX 2026-04-03: Validar CFOP manual contra formato permitido
            const cfopStr = String(cfopManual).trim();
            if (!/^[1-7]\d{3}$/.test(cfopStr)) {
                throw new Error(`CFOP manual inválido: "${cfopManual}". Formato esperado: 4 dígitos (ex: 5102, 6102).`);
            }
            return { cfop: cfopStr, descricao: 'CFOP manual', tipo: 'manual' };
        }
