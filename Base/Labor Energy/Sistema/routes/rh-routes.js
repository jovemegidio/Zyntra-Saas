...
module.exports = function createRHRoutes(deps) {
    ...
    // =========================================================================
    // ANIVERSARIANTES — GET /api/rh/funcionarios/aniversariantes?mes=N
    // NOTA: Esta rota DEVE vir ANTES de /funcionarios/:id para não ser capturada pelo param
    // =========================================================================
    router.get('/funcionarios/aniversariantes', asyncHandler(async (req, res) => {
        ...
        try {
            const [aniversariantes] = await pool.query(
                `SELECT id, nome_completo, data_nascimento, cargo, departamento, foto_perfil_url, email
                 FROM funcionarios
                 WHERE MONTH(data_nascimento) = ? AND (status = 'Ativo' OR status IS NULL)
        ...
        try {
            ...
            const [result] = await pool.query(`
                UPDATE funcionarios SET
                    ...
                    contato_emergencia_nome = COALESCE(?, contato_emergencia_nome),
                    contato_emergencia_telefone = COALESCE(?, contato_emergencia_telefone),
                    contato_emergencia_parentesco = COALESCE(?, contato_emergencia_parentesco)
                WHERE id = ?
            `, [
                ...
                emptyToNull(contato_emergencia_nome), emptyToNull(contato_emergencia_telefone), emptyToNull(contato_emergencia_parentesco),
                id
            ]);
    ...
    router.get('/avisos', async (req, res, next) => {
        try {
            ...
            try {
                await pool.query(`ALTER TABLE avisos ADD COLUMN tipo VARCHAR(50) DEFAULT 'info'`);
                await pool.query(`ALTER TABLE avisos ADD COLUMN conteudo TEXT`);
                await pool.query(`ALTER TABLE avisos ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP`);
    ...
    // =========================================================================
    // CONSENTIMENTO HOLERITES — GET /api/rh/holerites/consentimento
    // NOTA: Deve vir ANTES de /holerites/:id para não ser capturado pelo param
    // =========================================================================
    router.get('/holerites/consentimento', asyncHandler(async (req, res) => {
        const userId = req.user.id;
        try {
            const [tables] = await pool.query(
                "SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'rh_holerite_consentimento'"
            );
            if (tables.length === 0) {
                return res.json({ consentimento: true, admin: true });
            }
            const [rows] = await pool.query(
                `SELECT id, data_consentimento FROM rh_holerite_consentimento WHERE usuario_id = ? LIMIT 1`,
                [userId]
            );
            const isAdmin = req.user.is_admin || req.user.role === 'admin' || req.user.role === 'administrador';
            res.json({
                consentimento: rows.length > 0,
                admin: isAdmin,
                data_consentimento: rows.length > 0 ? rows[0].data_consentimento : null
            });
        } catch (error) {
            ...
            res.status(500).json({ message: 'Erro ao verificar consentimento' });
        }
    }));

    // POST /api/rh/holerites/consentimento — Registrar consentimento
    router.post('/holerites/consentimento', asyncHandler(async (req, res) => {
        const userId = req.user.id;
        try {
            await pool.query(`
                CREATE TABLE IF NOT EXISTS rh_holerite_consentimento (
                    id INT AUTO_INCREMENT PRIMARY KEY,
            ...
            `);
            await pool.query(
                `INSERT INTO rh_holerite_consentimento (usuario_id, ip_consentimento) VALUES (?, ?)
                 ON DUPLICATE KEY UPDATE data_consentimento = CURRENT_TIMESTAMP`,
                [userId, req.ip]
            );
    ...
    router.get('/avaliacoes/dashboard', authenticateToken, async (req, res) => {
        try {
            ...
            // Tabela inexistente: retornar dados zerados em vez de 500
            if (error.code === 'ER_NO_SUCH_TABLE') {
                return res.json({ avaliacoes_concluidas: 0, avaliacoes_pendentes: 0, periodos_ativos: 0, competencias_ativas: 0 });
            }
    ...
    // GET /api/rh/configuracoes/ponto-eletrônico (both formats: ponto-eletrônico and ponto-eletronico)
    router.get(['/configuracoes/ponto-eletrônico', '/configuracoes/ponto-eletronico'], async (req, res) => {
        try {
            const [rows] = await pool.query(
                "SELECT chave, valor FROM rh_configuracoes WHERE categoria IN ('ponto_eletronico', 'ponto_eletrônico')"
            );
            const config = {};
            rows.forEach(r => { config[r.chave] = r.valor; });
            res.json({
    ...
    // PUT /api/rh/configuracoes/ponto-eletrônico (both formats: ponto-eletrônico and ponto-eletronico)
    ...
    // =========================================================================
    // PENSÃO ALIMENTÍCIA — GET /api/rh/funcionarios/:id/pensao
    // Retorna pensões alimentícias cadastradas para um funcionário
    // =========================================================================
    router.get('/funcionarios/:id/pensao', [
        ...
        try {
            // Verificar se a tabela existe
            const [tables] = await pool.query(
                "SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'rh_pensao_alimenticia'"
            );
            if (tables.length === 0) {
            ...
            const [pensoes] = await pool.query(
                `SELECT id, funcionario_id, nome_recebedor, cpf_recebedor, 
                        valor,
            ...
            // Se a tabela não existe, retornar array vazio em vez de 500
            if (error.code === 'ER_NO_SUCH_TABLE') {
                return res.json([]);
            }
            console.error('Erro ao buscar pensões:', error);
            res.status(500).json({ message: 'Erro ao buscar pensões alimentícias' });
        }
    }));
