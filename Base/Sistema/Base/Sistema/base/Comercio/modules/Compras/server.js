...
// Listar NFs de Entrada
app.get('/api/compras/nf-entrada', authenticateToken, async (req, res) => {
    try {
        const limite = Math.min(Math.max(parseInt(req.query.limite) || 100, 1), 500);
        const [notas] = await mysqlPool.query(
            'SELECT * FROM nf_entrada ORDER BY created_at DESC LIMIT ?',
            [limite]
        );
        res.json({ notas });
    } catch (error) {
        console.error('[COMPRAS] Erro ao listar NFs de entrada:', error);
        res.status(500).json({ error: 'Erro ao listar NFs de entrada' });
    }
});
...
app.post('/api/compras/nf-entrada/importar-chave', authenticateToken, async (req, res) => {
    try {
        ...
        const serie = parseInt(chaveClean.substring(22, 25));
        const numero = parseInt(chaveClean.substring(25, 34));

        // Buscar fornecedor cadastrado
        const [forn] = await mysqlPool.query('SELECT razao_social, cnpj FROM fornecedores WHERE cnpj = ?', [cnpj]);
        const fornecedorRazao = forn.length > 0 ? forn[0].razao_social : '';
        const fornecedorCnpj = forn.length > 0 ? forn[0].cnpj : cnpj;

        // Inserir na nf_entrada
        const [ins] = await mysqlPool.query(`
            INSERT INTO nf_entrada (chave_acesso, numero_nfe, serie, modelo,
                fornecedor_cnpj, fornecedor_razao_social,
                status, importado_por)
            VALUES (?, ?, ?, ?, ?, ?, 'importada', ?)
        `, [chaveClean, numero, serie, modelo, fornecedorCnpj, fornecedorRazao, req.user?.id]);

        res.json({
            success: true, id: ins.insertId,
            numero_nfe: numero, serie, chave_acesso: chaveClean,
            fornecedor: { razao_social: fornecedorRazao, cnpj: fornecedorCnpj },
            data_emissao: new Date().toISOString(),
            message: `NF ${numero} importada com sucesso via chave de acesso`
        });
    } catch (error) {
...
app.get('/api/compras/dashboard', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            valor_total: parseFloat(totais[0].valor_total_pedidos) || 0,
            ...
            ultimos_pedidos: pedidosRecentes,
