...
router.post('/rhid/config', authenticateToken, async (req, res) => {
    try {
        ...
        if (existing.length > 0) {
            ...
            await pool.query(
                'INSERT INTO controlid_config (rhid_email, rhid_password, rhid_enabled, rhid_auto_sync, rhid_sync_interval, ativo) VALUES (?, ?, ?, ?, ?, TRUE)',
                [email, encPassword, isEnabled, isAutoSync, interval]
            );
        }
...
router.post('/rhid/test', authenticateToken, async (req, res) => {
    try {
        ...
        var devices = await rhidApiCall('customerdb/device', 'a', 'GET', null, email, password);
        var persons = await rhidApiCall('customerdb/person', 'a', 'GET', null, email, password);

        var deviceList = Array.isArray(devices) ? devices : [];
        var personList = Array.isArray(persons) ? persons : [];
...
router.post('/rhid/sync', authenticateToken, async (req, res) => {
    try {
        ...
        // 4. Buscar marcações detalhadas via apuracao_ponto (em batches de 20)
        var BATCH_SIZE = 20;
        var allRecords = [];
        ...
        for (var i = 0; i < personIds.length; i += BATCH_SIZE) {
            ...
            try {
                ...
                if (Array.isArray(apuracao)) {
                    for (var d = 0; d < apuracao.length; d++) {
                        var dayRecord = apuracao[d];
                ...
                console.error('[RHiD Sync] Erro no batch ' + i + '-' + (i + BATCH_SIZE) + ':', batchErr.message);
            }
        }
...
router.post('/import', authenticateToken, async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Erro ao importar registros' });
    }
});
