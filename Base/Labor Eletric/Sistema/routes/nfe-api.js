...
module.exports = function createNfeApiRouter({ authenticateToken, pool }) {
    ...
    // FIX-100: NF-e environment configurable via .env (1=production, 2=homologação)
    // Set NFE_AMBIENTE=1 in .env when A1 digital certificate is configured
    ...
    <det nItem="${parseInt(item.numero, 10) || idx + 1}">
      ...
      <imposto>
        <ICMS><ICMS${item.cst||'00'}><orig>${item.orig||'0'}</orig><CST>${item.cst||'00'}</CST>${(function(){var vBC=(parseFloat(item.valorTotal)||0).toFixed(2),pI=parseFloat(item.aliquota_icms||item.pICMS)||0;return pI>0?'<vBC>'+vBC+'</vBC><pICMS>'+pI.toFixed(2)+'</pICMS><vICMS>'+(parseFloat(vBC)*pI/100).toFixed(2)+'</vICMS>':''}())}</ICMS${item.cst||'00'}></ICMS>
      </imposto>
    </det>`;
            });
...
<NFe xmlns="http://www.portalfiscal.inf.br/nfe">
  <infNFe versao="4.00">
    ...
    // POST /api/nfe/emitir
    // AUDIT-FIX S4.5: Retry com exponential backoff para chamadas SEFAZ
    router.post('/emitir', authenticateToken, async (req, res) => {
        try {
            ...
            function callFaturamento(attempt) {
                    ...
                    const faturamentoReq = http.request({
                        ...
                        faturamentoRes.on('end', () => {
                            try {
                                ...
                                resolve({ statusCode: faturamentoRes.statusCode, result });
                            } catch {
                                reject({ code: 'PARSE_ERROR', statusCode: 502, message: 'Resposta inválida do serviço de faturamento.' });
                            }
                        });
                    });
                    faturamentoReq.on('error', (err) => {
                        reject({ code: 'FATURAMENTO_OFFLINE', statusCode: 503, message: err.message });
                    });
                    faturamentoReq.on('timeout', () => {
                        faturamentoReq.destroy();
                        reject({ code: 'TIMEOUT', statusCode: 504, message: 'Timeout ao conectar com serviço de faturamento.' });
                    });
            ...
            for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                try {
                    const { statusCode, result } = await callFaturamento(attempt);
                    // Sucesso ou erro de negócio (4xx) — não fazer retry
                    if (statusCode < 500) {
                        if (attempt > 1) console.log(`[NFe Emitir] ✅ Sucesso na tentativa ${attempt}`);
                        return res.status(statusCode).json(result);
                    }
                    // Erro de servidor (5xx) — retry
                    lastError = { code: 'SERVER_ERROR', statusCode, result };
                    console.warn(`[NFe Emitir] ⚠️ Tentativa ${attempt}/${MAX_RETRIES} falhou (HTTP ${statusCode})`);
                } catch (err) {
                    lastError = err;
                    console.warn(`[NFe Emitir] ⚠️ Tentativa ${attempt}/${MAX_RETRIES} falhou: ${err.code} - ${err.message}`);
                }

                // Exponential backoff (exceto última tentativa)
            ...
            // Todas tentativas falharam — registrar para DLQ/recovery
            console.error(`[NFe Emitir] 🔴 Todas ${MAX_RETRIES} tentativas falharam. NFe pendente para retry manual.`, {
                nfe_id: nfeData.id || 'N/A',
                cliente: nfeData.destinatario?.nome || 'N/A',
                lastError: lastError?.code
            });

            // Responder com status do último erro
            const status = lastError?.statusCode || 503;
            const result = lastError?.result || {
                success: false,
                message: `Serviço de faturamento indisponível após ${MAX_RETRIES} tentativas. Tente novamente mais tarde.`,
                code: lastError?.code || 'FATURAMENTO_OFFLINE',
    ...
    // FIX-100: POST /cancelar — NF-e cancellation with 24h SEFAZ window
    router.post('/cancelar', authenticateToken, async (req, res) => {
        try {
            const { nfe_id, chave_acesso, justificativa } = req.body;
            if (!nfe_id && !chave_acesso) {
                return res.status(400).json({ success: false, message: 'ID ou chave de acesso obrigatório.' });
            }
            if (!justificativa || justificativa.trim().length < 15) {
                return res.status(400).json({ success: false, message: 'Justificativa obrigatória (mín. 15 caracteres).' });
            }
            let nfe = null;
            for (const t of ['notas_fiscais', 'nfes']) {
                try {
                    const field = nfe_id ? 'id' : 'chave_acesso';
                    const [rows] = await pool.query('SELECT id, data_emissao, status, chave_acesso FROM ' + t + ' WHERE ' + field + ' = ? LIMIT 1', [nfe_id || chave_acesso]);
                    if (rows.length) { nfe = { ...rows[0], _t: t }; break; }
                } catch (_) {}
            }
            if (!nfe) return res.status(404).json({ success: false, message: 'NF-e não encontrada.' });
            if (nfe.status === 'cancelada') return res.status(400).json({ success: false, message: 'NF-e já cancelada.' });

            const horas = (Date.now() - new Date(nfe.data_emissao).getTime()) / 3600000;
            if (horas > 24) {
                return res.status(400).json({
                    success: false,
                    message: 'Prazo excedido (' + horas.toFixed(1) + 'h). Limite: 24h. Use Carta de Correção.',
                    horasDecorridas: +horas.toFixed(1)
                });
            }
            await pool.query('UPDATE ' + nfe._t + ' SET status = ?, data_cancelamento = NOW(), justificativa_cancelamento = ? WHERE id = ?',
                ['cancelada', justificativa.trim(), nfe.id]);

            res.json({ success: true, message: 'NF-e cancelada com sucesso.', nfe_id: nfe.id, horasDecorridas: +horas.toFixed(1) });
        } catch (err) {
            console.error('[NFe Cancelar] Erro:', err);
            res.status(500).json({ success: false, message: 'Erro ao cancelar NF-e.' });
        }
    });

    // FIX-100: GET /proximo-numero — Sequential NF-e number with FOR UPDATE lock
    router.get('/proximo-numero', authenticateToken, async (req, res) => {
        let conn;
        try {
            ...
            let maxN = 0;
            for (const sql of [
                'SELECT COALESCE(MAX(CAST(numero AS UNSIGNED)),0) AS m FROM notas_fiscais WHERE serie = ? FOR UPDATE',
                'SELECT COALESCE(MAX(CAST(numero AS UNSIGNED)),0) AS m FROM nfes WHERE serie = ?'
            ]) {
                try {
                    const [r] = await conn.query(sql, [serie]);
                    if (r[0] && r[0].m > maxN) maxN = r[0].m;
                } catch (_) {}
            }
            await conn.commit();
            res.json({ success: true, numero: maxN + 1, serie });
        } catch (err) {
            if (conn) try { await conn.rollback(); } catch (_) {}
            console.error('[NFe ProximoNumero] Erro:', err);
            res.status(500).json({ success: false, message: 'Erro ao obter próximo número.' });
        } finally {
