...
class AuthenticationError extends AppError {
  constructor(message = 'Falha na autenticação. Faça login novamente.') {
    super(message, 401);
  }
}
...
class DatabaseError extends AppError {
  constructor(message = 'Erro ao processar a operação. Tente novamente.') {
    super(message, 500);
  }
}
...
const errorHandler = (err, req, res, next) => {
  ...
  if (err.code === 'ER_DUP_ENTRY') {
    const message = 'Registro duplicado. Já existe um item com esses dados.';
    error = new ValidationError(message);
  }

  // MySQL foreign key constraint error
  if (err.code === 'ER_NO_REFERENCED_ROW_2') {
    const message = 'Registro referenciado não encontrado.';
    error = new ValidationError(message);
  }
  ...
  if (err.name === 'ValidationError') {
    ...
    error = new ValidationError('Erro de validação', message);
  }
...
const notFound = (req, res, next) => {
  const error = new NotFoundError('Página não encontrada.');
  next(error);
};
