 ...
 */
const RATE_LIMITS = {
    // Login/Autenticação - mais restritivo (prevenir brute force)
    auth: {
...
function categorizeRoute(path, method) {
    ...
    // Autenticação - manter categoria dedicada para prevenir brute force
    if (pathLower.includes('login') || pathLower.includes('logout') ||
        pathLower.includes('auth') || pathLower.includes('password') ||
        pathLower.includes('refresh') || pathLower.includes('token')) {
        return 'auth';
    }
