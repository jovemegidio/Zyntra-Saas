#!/bin/bash
# ============================================
# CLONAR ESTRUTURA DO BANCO + INSERIR DADOS DEMO
# ============================================
set -e
...
mysqldump $MYSQL_CRED --no-data --skip-triggers --skip-lock-tables \
  --single-transaction --set-gtid-purged=OFF \
  $SOURCE_DB 2>/dev/null > /tmp/structure.sql

if [ $? -ne 0 ] || [ ! -s /tmp/structure.sql ]; then
    echo "⚠️ mysqldump falhou, tentando método alternativo..."
    # Método alternativo: pegar CREATE TABLE de cada tabela
    for table in $(mysql $MYSQL_CRED -N -e "SELECT table_name FROM information_schema.tables WHERE table_schema='$SOURCE_DB' AND table_type='BASE TABLE';" 2>/dev/null); do
        mysql $MYSQL_CRED -N -e "SHOW CREATE TABLE $SOURCE_DB.\`$table\`\G" 2>/dev/null | \
        grep -v "^\*\*\*" | sed "s/Create Table: //" >> /tmp/structure.sql
        echo ";" >> /tmp/structure.sql
    done
fi

echo "📥 Importando estrutura no $DEMO_DB..."
mysql $MYSQL_CRED $DEMO_DB < /tmp/structure.sql 2>/dev/null || true

TABLES=$(mysql $MYSQL_CRED -N -e "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='$DEMO_DB' AND table_type='BASE TABLE';" 2>/dev/null)
echo "✅ Tabelas clonadas: $TABLES"

echo ""
echo "👤 Inserindo dados de demonstração..."

# Dados demo em arquivo separado para evitar problemas de encoding
cat > /tmp/demo-data.sql << 'SQLEOF'
-- ========================
...
(5, 'PROD-005', 'Rack 44U Fechado', 'Rack padrão 19pol 44U 800x1000mm', 'UN', 2890.00, 1450.00, 5, 12, '73269090', 'ativo'),
...
(5, 'Aluguel galpão Mar/2026', NULL, 15000.00, '2026-03-05', '2026-03-04', 'pago', 'aluguel'),
...
(3, 'OP-2026-003', 7, 2, 2000, '2026-02-28', '2026-03-12', 'planejada', 'media', 'Cabos ópticos'),
(4, 'OP-2026-004', 8, 4, 3, '2026-03-05', '2026-03-12', 'planejada', 'media', 'Switches'),
(5, 'OP-2026-005', 9, 9, 10, '2026-03-06', '2026-03-18', 'planejada', 'normal', 'Câmeras');
...
(2, 'Vendas', 'Módulo de vendas', 'shopping_cart', '/vendas'),
(3, 'Financeiro', 'Módulo financeiro', 'account_balance', '/financeiro'),
(4, 'PCP', 'Produção', 'precision_manufacturing', '/pcp'),
...
mysql $MYSQL_CRED $DEMO_DB < /tmp/demo-data.sql 2>&1

echo ""
echo "📊 Resumo:"
mysql $MYSQL_CRED $DEMO_DB -e "
...
UNION ALL SELECT 'Pedidos', COUNT(*) FROM pedidos;" 2>/dev/null

echo ""
echo "✅ ZYNTRA DEMO DATABASE PRONTO!"
echo "🔑 admin@zyntra.com.br / Demo@2026!"
