...
// Função para converter data do Excel (número serial) para Date
...
// Função para limpar e normalizar valores
...
// Função para mapear colunas do Excel para campos do banco
function mapearLinha(row, colunas) {
    ...
    const situacao = getValue(['Situação']);
    const ativo = situacao ? (situacao.toLowerCase() === 'ativo' ? 1 : 0) : 1;

    return {
        tags: getValue(['Tags']),
        razao_social: getValue(['Razão Social / Nome Completo', 'Razão Social']),
        ...
        cidade,
        estado,
        endereco: getValue(['Endereço']),
        bairro: getValue(['Bairro']),
        cep: getValue(['CEP']),
        banco: getValue(['Banco']),
        agencia: getValue(['Agência']),
        conta_corrente: getValue(['Conta Corrente']),
        inscricao_estadual: getValue(['Inscrição Estadual']),
        contribuinte_icms: simNaoToBoolean(getValue(['Contribuinte do ICMS'])),
        inscricao_municipal: getValue(['Inscrição Municipal']),
        tipo_atividade: getValue(['Tipo de Atividade']),
        vendedor_responsavel: getValue(['Vendedor (padrão)']),
        credito_total: parseMonetario(getValue(['Crédito Total'])),
        total_a_receber: parseMonetario(getValue(['Total a Receber'])),
        credito_disponivel: parseMonetario(getValue(['Crédito Disponível'])),
        transportadora: getValue(['Transportadora']),
        data_cadastro: formatDateForMySQL(excelDateToJS(getValue(['Inclusão']))),
        data_ultima_alteracao: formatDateForMySQL(excelDateToJS(getValue(['Última Alteração']))),
        incluido_por: getValue(['Incluído por']),
        alterado_por: getValue(['Alterado por']),
        ativo
    };
}

// Função para ler clientes do Excel
function lerClientesExcel(arquivo) {
    const caminho = path.join(pastaClientes, arquivo.nome);

    if (!fs.existsSync(caminho)) {
        console.log(`❌ Arquivo não encontrado: ${arquivo.nome}`);
        return [];
    }
    ...
    // Encontrar linha do cabeçalho (procurar por "CNPJ" ou "Razão Social")
    let headerRow = -1;
    for (let i = 0; i < Math.min(10, data.length); i++) {
        const row = data[i];
        if (row && row.some(cell => cell && (
            String(cell).includes('CNPJ') ||
            String(cell).includes('Razão Social') ||
            String(cell).includes('Situação')
        ))) {
            headerRow = i;
            break;
        }
    ...
    if (headerRow === -1) {
        console.log(`❌ Cabeçalho não encontrado em: ${arquivo.nome}`);
        return [];
    }
...
async function main() {
    console.log('╔══════════════════════════════════════════════════════════════╗');
    console.log('║     IMPORTAÇÍO DE CLIENTES - EXCEL → MySQL (Local + Railway)  ║');
    console.log('╚══════════════════════════════════════════════════════════════╝');
    ...
    try {
        ...
        console.log('   ✅ Banco RAILWAY conectado');

        // Adicionar índice único para CNPJ/CPF + empresa_id se não existir
        try {
            await localConn.query(`
                ALTER TABLE clientes
                ADD UNIQUE INDEX idx_cnpj_empresa (cnpj_cpf, empresa_id)
            `);
            console.log('   📋 Índice único criado no banco LOCAL');
        } catch (e) {
            // Índice já existe
        }
        ...
        try {
            await railwayConn.query(`
                ...
                ADD UNIQUE INDEX idx_cnpj_empresa (cnpj_cpf, empresa_id)
            `);
            console.log('   📋 Índice único criado no banco RAILWAY');
        } catch (e) {
            // Índice já existe
        }
        ...
        for (const arquivo of arquivos) {
            console.log(`📄 Processando: ${arquivo.nome}`);
            ...
            for (const cliente of clientes) {
                ...
                try {
                    ...
                    if (!err.message.includes('Duplicate')) {
                        console.error(`   ❌ LOCAL - Erro: ${cliente.cnpj_cpf} - ${err.message}`);
                    }
                }
                ...
                try {
                    ...
                    stats.railway.erros++;
                    if (!err.message.includes('Duplicate')) {
                        console.error(`   ❌ RAILWAY - Erro: ${cliente.cnpj_cpf} - ${err.message}`);
                    }
                }
            }

            console.log(`   ✅ LOCAL: ${localOk} processados`);
            console.log(`   ✅ RAILWAY: ${railwayOk} processados`);
        }
        ...
        console.log(`   ➕ Inseridos: ${stats.local.inseridos}`);
        console.log(`   🔄 Atualizados: ${stats.local.atualizados}`);
        console.log(`   ❌ Erros: ${stats.local.erros}`);

        console.log('📊 BANCO RAILWAY:');
        console.log(`   ➕ Inseridos: ${stats.railway.inseridos}`);
        console.log(`   🔄 Atualizados: ${stats.railway.atualizados}`);
        console.log(`   ❌ Erros: ${stats.railway.erros}`);
        ...
        console.log('📈 TOTAL DE CLIENTES NOS BANCOS:');
        console.log(`   🏠 LOCAL: ${localTotal[0].total}`);
        console.log(`   ☁️  RAILWAY: ${railwayTotal[0].total}`);
